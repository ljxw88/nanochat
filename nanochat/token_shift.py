...
class ShiftLinear(nn.Module):
    ...
    def forward(self, x: torch.Tensor, shift_state: Optional[torch.Tensor] = None) -> torch.Tensor:
        ...
        if T > 1:
            # Training/prefill: use token_shift with proper weight shapes
            result_per_head = token_shift(out_per_head, prev_weight, curr_weight)
        else:
            # Decoding with cache: blend with previous state
            if shift_state is not None:
                shift_state_per_head = rearrange(shift_state, 'b (h d) -> b 1 h d', h=self.num_heads)
                # alpha has shape (B, 1, H), need to add dimension for broadcasting with (B, 1, H, D)
                result_per_head = weight_sum(shift_state_per_head, out_per_head, alpha[..., None])
            else:
                # No shift state available, just use current output
                result_per_head = out_per_head

        result_per_head = result_per_head.to(out.dtype)

        # Update shift state with the last token's output for next iteration
        if shift_state is not None:

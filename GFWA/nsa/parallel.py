...
def _sliding_window_attention_with_gfwa(
    ...
    *,
    ...
    def _prepare_log_fgate(start: Optional[int] = None, end: Optional[int] = None) -> Optional[torch.Tensor]:
        if log_fgate is None:
            return None
        if log_fgate.ndim != 3:
            raise ValueError(f"log_fgate must be 3D, got shape {tuple(log_fgate.shape)}")

        # log_fgate can be provided either in [B, T, H] or [B, H, T] layout.
        if log_fgate.shape[1] == q.shape[1] and log_fgate.shape[2] == q.shape[2]:
            # [B, T, H]
            fgate_slice = log_fgate[:, start:end] if start is not None else log_fgate
            fgate_slice = fgate_slice.permute(0, 2, 1).contiguous()
        elif log_fgate.shape[1] == q.shape[2] and log_fgate.shape[2] == q.shape[1]:
            # [B, H, T]
            fgate_slice = log_fgate[:, :, start:end] if start is not None else log_fgate
            fgate_slice = fgate_slice.contiguous()
        else:
            raise ValueError(
                f"Unexpected log_fgate shape {tuple(log_fgate.shape)}; "
                f"expected either [B, T, H] or [B, H, T] with T={q.shape[1]}, H={q.shape[2]}"
            )
        return fgate_slice.to(torch.float32)

    if cu_seqlens is not None:
        boundaries = cu_seqlens.tolist()
        assert len(boundaries) >= 2, "cu_seqlens must have at least two elements"
        out = torch.zeros(q.shape[0], q.shape[1], q.shape[2], v.shape[-1], dtype=v.dtype, device=v.device)
        for start, end in zip(boundaries[:-1], boundaries[1:]):
            if end <= start:
                continue
            q_slice = q[:, start:end].permute(0, 2, 1, 3).contiguous()
            k_slice = k[:, start:end].repeat_interleave(num_key_value_groups, dim=2).permute(0, 2, 1, 3).contiguous()
            v_slice = v[:, start:end].repeat_interleave(num_key_value_groups, dim=2).permute(0, 2, 1, 3).contiguous()
            log_fgate_slice = _prepare_log_fgate(start, end)
            o_slice = gfwa_flash_attention(
                q_slice,
                k_slice,
                v_slice,
                log_fgate=log_fgate_slice,
                head_first=True,
                sm_scale=scale,
                window_size=window_size,
            )
            out[:, start:end] = o_slice.permute(0, 2, 1, 3)
        return out

    q_hf = q.permute(0, 2, 1, 3).contiguous()
    k_hf = k.repeat_interleave(num_key_value_groups, dim=2).permute(0, 2, 1, 3).contiguous()
    v_hf = v.repeat_interleave(num_key_value_groups, dim=2).permute(0, 2, 1, 3).contiguous()
    log_fgate_hf = _prepare_log_fgate()
    o_hf = gfwa_flash_attention(
        q_hf,
        k_hf,
        v_hf,
        log_fgate=log_fgate_hf,
        head_first=True,
        sm_scale=scale,
        window_size=window_size,
    )
    return o_hf.permute(0, 2, 1, 3)
...
def parallel_nsa(
    ...
    Args:
        ...
        log_fgate (torch.Tensor, optional):
            Forgetting gate in log-domain applied to the sliding-window GFWA branch.
            Accepts shapes `[B, T, HQ]` or `[B, HQ, T]`.
    ...
    if window_size > 0:
        if g_swa is None:
            raise ValueError("g_swa must be provided when window_size > 0")
        o_swa = _sliding_window_attention_with_gfwa(
            q=q,
            k=k,
            v=v,
            window_size=window_size,
